...
description: Reviews conventional WordPress plugin code for architecture, security, settings/admin pages, custom content types, REST/AJAX, database access, capabilities, nonces, performance, accessibility, and production readiness. Use for plugin code that is not an MU-plugin. Works from an attached or selected project directory without requiring individual child-file attachments.
metadata:
  author: riley-childs
  version: "1.2"
---
